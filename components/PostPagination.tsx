...
const PostPagination: React.FC<PostPaginationProps> = ({ className, post, isNext }) => {
  const arrowBoxClassName = "bg-duchs-200 group-hover:bg-duchs-800 flex items-center justify-center h-14 mx-5 first:ml-0 last:mr-0 px-3 rounded-full transition-all w-14";
  ...
  return (
    <div className={className}>
      <Link href={post.ref}>
        <a className={"dark:hover:bg-neutral-600 hover:bg-neutral-300 flex group items-center rounded-full transition-all" + (isNext ? " pr-10" : " pl-10 justify-end")}>
